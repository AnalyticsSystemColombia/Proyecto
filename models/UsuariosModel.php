<?php 
	...
	public function insertUsuario(string $identificacion, string $nombre, string $apellido, int $telefono, string $email, string $password, int $tipoid, int $status){
		...
		if(empty($request))
		{
			$query_insert  = "INSERT INTO personas(identificacion,nombres,apellidos,telefono,email,password,rolid,status) 
								VALUES(?,?,?,?,?,?,?,?)";
	...
	public function selectUsuarios(){
		...
		if($_SESSION['idUser'] != 3 ){
			$whereAdmin = " and p.idpersona != 3 ";
		}
		$sql = "SELECT p.idpersona,p.identificacion,p.nombres,p.apellidos,p.telefono,p.email,p.status,r.idrol,r.roleNomb 
				FROM personas p 
				INNER JOIN roles r
				ON p.rolid = r.idrol
	...
	public function selectUsuario(int $idpersona)
	{
		...
		$sql = "SELECT p.idpersona,p.identificacion,p.nombres,p.apellidos,p.telefono,p.email,p.nit,p.nombrefiscal,p.direccionfiscal,r.idrol,r.roleNomb,p.status, DATE_FORMAT(p.persFech, '%d-%m-%Y') as fechaRegistro 
				FROM personas p
				INNER JOIN roles r
				ON p.rolid = r.idrol
	...
	public function updateUsuario(int $idUsuario, string $identificacion, string $nombre, string $apellido, int $telefono, string $email, string $password, int $tipoid, int $status){
		...
		if(empty($request))
		{
			if($this->strPassword  != "")
			{
				$sql = "UPDATE personas SET identificacion=?, nombres=?, apellidos=?, telefono=?, email=?, password=?, rolid=?, status=? 
				...
				$sql = "UPDATE personas SET identificacion=?, nombres=?, apellidos=?, telefono=?, email=?, rolid=?, status=? 
						WHERE idpersona = $this->intIdUsuario ";
				$arrData = array($this->strIdentificacion,
								$this->strNombre,
								$this->strApellido,
	...
	public function updatePerfil(int $idUsuario, string $identificacion, string $nombre, string $apellido, int $telefono, string $password){
		...
		$this->strPassword = $password;
		if($this->strPassword != ""){
			$sql = "UPDATE personas SET identificacion=?, nombres=?, apellidos=?, telefono=?, password=? 
					WHERE idpersona = $this->intIdUsuario ";
			$arrData = array($this->strIdentificacion,
							$this->strNombre,
							$this->strApellido,
			...
			$sql = "UPDATE personas SET identificacion=?, nombres=?, apellidos=?, telefono=? 
					WHERE idpersona = $this->intIdUsuario ";
			$arrData = array($this->strIdentificacion,
							$this->strNombre,
							$this->strApellido,
	...
	public function updateDataFiscal(int $idUsuario, string $strNit, string $strNomFiscal, string $strDirFiscal){
		...
		$sql = "UPDATE personas SET nit=?, nombrefiscal=?, direccionfiscal=? 
					WHERE idpersona = $this->intIdUsuario ";
